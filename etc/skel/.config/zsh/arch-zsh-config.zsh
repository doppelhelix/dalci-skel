#
# $XDG_CONFIG_HOME/zsh/arch-zsh-config.zsh
#
...
# If the history needs to be trimmed this option will cause the oldest to be lost before losing a unique event from the list.
...
ZSH_COMPDUMP="$XDG_CACHE_HOME"/zsh/zcompcache
[ -d $ZSH_COMPDUMP ] || mkdir -p $ZSH_COMPDUMP
autoload compinit
if [[ -r "$ZSH_COMPDUMP/zcompdump-${ZSH_VERSION}"*(#qN) ]] ; then
...
HISTTIMEFORMAT="[%F %T] "
HISTSIZE=10000000
# History filesize (bigger than histsize for HIST_IGNORE_ALL_DUPS)
SAVEHIST=100000000
...
function zsh_urlencode() {
    ...
    # Implemented to use a single printf call and
    # avoid subshells in the loop, for performance
    local url_str=""
    for (( i = 1; i <= ${#str}; ++i )); do
        byte="$str[i]"
